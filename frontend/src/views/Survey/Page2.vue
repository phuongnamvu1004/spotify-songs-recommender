  ...
  <div>
    ...
    <div class="relative z-10 mx-auto max-w-3xl">
      <div class="mb-8">
        ...
        <h1
          class="animate-gradient-x mb-8 bg-gradient-to-r from-green-400 via-[#1DB954] to-blue-500 bg-clip-text text-2xl font-bold text-transparent md:text-5xl"
          id="question-text"
        >
      ...
      >
        ...
        <div v-if="isSelected('G')" class="col-span-2 mt-4">
          <div class="rounded-lg border border-gray-500 p-4">
            <label for="custom-artist" class="mb-2 block text-lg text-white"
              >Please specify your favorite artists:</label
            >

            <!-- Artist chips/tags display -->
            <div
              v-if="customArtists.length > 0"
              class="mb-3 flex flex-wrap gap-2"
            >
              <div
                v-for="(artist, index) in customArtists"
                :key="index"
            ...
            >
              ...
              {{ customArtists.length === 1 ? "artist" : "artists" }} added
            </div>
      ...
      <div class="mt-6 flex items-center justify-between">
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            class="flex h-10 w-10 items-center justify-center rounded-md bg-gray-700 transition-colors hover:bg-[#77d397]"
            :class="{
              'opacity-50 cursor-not-allowed': !isNextEnabled || isLoading,
            }"
...
export default {
  ...
  computed: {
    isNextEnabled() {
      ...
      const hasStandardSelection = this.selectedAnswers.some(
        (option) => option !== "G"
      );
      const hasValidCustomSelection =
        this.isSelected("G") && this.customArtists.length > 0;
    ...
    totalSelectedArtists() {
      ...
      const standardCount = this.selectedAnswers.filter(
        (option) => option !== "G"
      ).length;
  ...
  methods: {
    ...
    async goToPage3() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to post artist preferences: ${response.status}`
          );
        }
    ...
    isDuplicateArtist() {
      ...
      return this.customArtists.some(
        (artist) =>
